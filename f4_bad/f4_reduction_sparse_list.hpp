...
public:
  ...
  */
  ...
  /** @brief reduces polynomials */
  void reduce_by_new();
  void reduce_some_by_one_new(
      unsigned i, const Prime_Field & F, const set<unsigned> & to_reduce
  );
  ///@}
  /** @name I/O */
  ///@{
...
protected:
  /**
    @brief reduces the specified set of rows by given polynomial,
      suitable for multithreading which actually reduces this polynomial!
  */
  void reduce_my_rows(
    const set<unsigned> &, const list<Entry *> &,
    vector<list<Entry *>::iterator> &
  );
