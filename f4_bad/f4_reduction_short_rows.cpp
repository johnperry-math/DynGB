...
void F4_Reduction_Data::reduce_my_rows(
  ...
  for (unsigned k : my_rows) {
    ...
    for (unsigned i = 0; i < Ak.size(); ++i) {
      ...
      unsigned mi = offset[k] + i;
      if (R[mi] != nullptr and Ak[i] != 0) {
        ...
        while (not gi->fellOff()) {
          ...
          Ak[l] += a*gi->currCoeff().value(); Ak[l] %= mod;
          if (was_zero)
            ++new_nonzero_entries;
          else if (Ak[l] == 0)
            --new_nonzero_entries;
...
void F4_Reduction_Data::reduce_by_new() {
  ...
  for (unsigned i = 0; i < num_rows; ++i) {
    ...
    if (nonzero_entries[i] > 0) {
      for (unsigned j = head[i]; j < Ai.size(); ++j) {
        bool searching = true;
        unsigned k = 0;
        unsigned l = j + offset[i];
        while (searching and k < num_rows) {
          if (i != k and nonzero_entries[k] > 0)
            searching = (l - offset[k] != head[k] or Ai[j] == 0);
          ++k;
        }
        ...
        if (not searching) {
          ...
          unsigned k0 = 0;
          auto a = (Ai[j] * (mod - F.inverse(Ak[head[k]]))) % mod;
          while (l - offset[i] < Ai.size() and k0 < nonzero_entries[k]) {
            unsigned kl = l - offset[k];
            if (Ak[kl] != 0) {
              ++k0;
              unsigned il = l - offset[i];
              bool was_zero = (Ai[il] == 0);
              Ai[il] += a*Ak[kl]; Ai[il] %= mod;
              if (was_zero)
                ++new_nonzero_entries;
              else if (Ai[il] == 0)
                --new_nonzero_entries;
