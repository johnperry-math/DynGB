...
void F4_Reduction_Data::initialize_many(const list<Critical_Pair_Basic *> & P) {
  ...
  for (auto cp : P) {
    ...
    A[row].resize(2*p->length(), F0);
    I[row].resize(2*p->length(), 0);
    nonzero_entries[row] = 0;
    unsigned j = 0;
    //cout << "new poly " << *p << endl;
    for (unsigned i = 0; not pi->fellOff(); ++i) {
      //cout << "starting search for " << pi->currMonomial() * t << " at " << i << ',' << *(M[i]) << endl;
      while (not M[i]->like_multiple(pi->currMonomial(), t))
        ++i;
      A[row][j] = pi->currCoeff();
      I[row][j] = i;
      ++j;
      pi->moveRight();
      ++nonzero_entries[row];
...
void F4_Reduction_Data::reduce_my_rows(const set<unsigned> & my_rows) {
  ...
  // expansion
  vector<Prime_Field_Element> buffer(num_cols, F.zero());
  unsigned buffer_head = num_cols;
  ...
  for (unsigned k : my_rows) {
    ...
    for (unsigned i = 0; i < nonzero_entries[k]; /* */) {
      ...
      if (R[mi] == nullptr)
        ...
        Prime_Field_Element a(-(A[k][i])*gi->currCoeff().inverse());
        ...
        while (l < nonzero_entries[k] and not gi->fellOff()) {
          ...
          while (l < nonzero_entries[k] and I[k][l] < j) {
            buffer[buffer_loc] = A[k][l];
            indices[buffer_loc] = I[k][l];
            ++l; ++buffer_loc;
          }
          if (l < nonzero_entries[k]) {
            if (I[k][l] > j) {
              ...
              buffer[buffer_loc] = A[k][l] + a*gi->currCoeff();
        ...
        delete gi;
        if (A[k].capacity() < i + buffer_loc) {
          A[k].resize(i + buffer_loc, F.zero());
          I[k].resize(i + buffer_loc, 0);
        }
        for (unsigned j = 0; j < buffer_loc; ++j) {
          A[k][i + j] = buffer[j];
          I[k][i + j] = indices[j];
        }
...
void F4_Reduction_Data::reduce_by_new() {
  vector<Prime_Field_Element> buffer(num_cols, Rx.ground_field().zero());
    ...
    if (nonzero_entries[i] > 0) {
      for (unsigned j = 0; j < nonzero_entries[i]; /* */) {
        ...
        if (searching)
          ...
          auto a = A[i][il] * (-A[k][kl].inverse());
          while (il < nonzero_entries[i] and kl < nonzero_entries[k]) {
            if (I[i][il] < I[k][kl]) {
              buffer[buffer_loc] = A[i][il];
              indices[buffer_loc] = I[i][il];
              ++buffer_loc; ++il;
            } else if (I[i][il] > I[k][kl]) {
              buffer[buffer_loc] = a*A[k][kl];
              indices[buffer_loc] = I[k][kl];
              ++buffer_loc; ++kl;
              ++new_nonzero_entries;
            } else {
              buffer[buffer_loc] = A[i][il] + a*A[k][kl];
              if (buffer[buffer_loc].is_zero()) {
                --new_nonzero_entries;
              } else {
                indices[buffer_loc] = I[i][il];
          ...
          if (A[i].capacity() < j + buffer_loc) {
            A[i].resize(2*(j + buffer_loc), Rx.ground_field().zero());
            I[i].resize(2*(j + buffer_loc), 0);
          }
