...
static time_t lock_times[4] { 0, 0, 0, 0 };

void F4_Reduction_Data::build_reducer(unsigned buffer, unsigned mi) {
  //print_lock.lock(); cout << buffer << " is building reducer for monomial #" << mi << endl; print_lock.unlock();
  const auto g = R[mi];
  ...
  while (not all_done) {
    ...
    for (k = r.size() - 1; k > 0; --k) {
      auto rk = r[k].first;
      if (R[rk] != nullptr) {
        bool have_lock = red_mutex[rk].try_lock();
        if (have_lock) {
          if (R_built[rk].size() == 0) build_reducer(buffer, rk);
          red_mutex[rk].unlock();
        } else undone.insert(rk);
      }
    }
  }
  set<unsigned> done;
  ...
  lock_times[buffer] += difftime(stop, start);
  expand(r, RB[buffer], Rprev[buffer], Rnext[buffer]);
  auto mod = Rx.ground_field().modulus();
  unsigned head = r[0].first, nonzero_entries = r.size();
  for (k = 1; k < r.size(); ++k) {
    auto rk = r[k].first;
    if (R[rk] != nullptr) {
      reduce_monomial(
        RB[buffer], R_built[rk], r[k].second, mod, rk, head,
        Rprev[buffer], Rnext[buffer], nonzero_entries
      );
    }
  }
  condense(r, head, RB[buffer], Rnext[buffer], nonzero_entries);
}

void F4_Reduction_Data::reduce_my_rows(
    const vector<int> & my_rows, unsigned buffer,
    vector<unsigned> & prev, vector<unsigned> & next
) {
  NVAR_TYPE n = Rx.number_of_variables();
  ...
  for (unsigned i : my_rows) {
    ...
    for (unsigned j = head; j < num_cols; /* */) {
      ...
      if (g == nullptr)
        ...
        red_mutex[j].lock();
        auto & r = R_built[j];
        if (r.size() == 0) build_reducer(buffer, j);
        red_mutex[j].unlock();
        auto si = strategies[i];
        si->pre_reduction_tasks(u, *g);
        j = reduce_monomial(
            B[buffer], r, a, mod, j, head, prev, next, nonzero_entries[i]
        );
      }
    }
    condense(A[i], head, B[buffer], next, nonzero_entries[i]);
  }
  print_lock.lock(); cout << "first reduction thread " << buffer << " terminated\n"; print_lock.unlock();
}
...
void F4_Reduction_Data::reduce_by_old() {
  ...
  RB.resize(num_threads);
  Rprev.resize(num_threads);
  Rnext.resize(num_threads);
  for (unsigned c = 0; c < num_threads; ++c) {
    B[c].resize(num_cols);
    RB[c].resize(num_cols);
    Rprev[c].resize(num_cols);
    Rnext[c].resize(num_cols);
    workers[c] = thread(
        &F4_Reduction_Data::reduce_my_rows, this, std::cref(thread_rows[c]),
        c, std::ref(prev[c]), std::ref(next[c])
    );
  }
  for (unsigned c = 0; c < num_threads; ++c)
    workers[c].join();
  ...
  for (unsigned k = 0; k < num_rows; ++k)
    check_consistency(k);*/
  time_t total = lock_times[0] + lock_times[1] + lock_times[2] + lock_times[3];
  cout << "spent " << total << " seconds in lock contention while building reducers\n";
}
