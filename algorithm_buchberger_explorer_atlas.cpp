...
/**
  ...
  @details The first entry in the arrays needs to identify the processor.
    (This is ignored by the function, but needed later.)
...
void hilbert_chooser(
  ...
  int64_t i;
  int64_t n = first[1];
  for (i = 2; undecided and i < n + 2; ++i)
    undecided = keep_second = (first[i] < second[i]);
  if (undecided)
    for (/* i already assigned */ ; undecided and i < *len; ++i)
      undecided = keep_second = (first[i] == second[i]);
...
list<Constant_Polynomial *> buchberger_explorer(
  ...
  while (min_todo != 0) {
    ...
    if (p_in.first != XPLOR_GENERATOR) {
      if (p_in.second == XPLOR_GENERATOR)
        ...
        R.push_back(new Critical_Pair_XPlor(p_in.first, p_in.second, p_in.sugar, G));
    }
    //MPI_Barrier(MPI_COMM_WORLD);
    for (Critical_Pair_XPlor * p : R) {
      // make s-poly
  ...
  if (comm_id == 0) {
    ...
    cout << max_bcast_time << " seconds spent in communication\n";
    // cleanup
