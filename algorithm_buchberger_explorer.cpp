...
#define HILBERT_COMPARISON 0
#define SUGAR_COMPARISON 1
#define WHICH_COMPARISON SUGAR_COMPARISON
...
/**
  @brief usable by @c MPI_Allreduce() to determine minimial sugar,
    breaking ties by minimal leading monomial
*/
void sugar_chooser(
  void * firstin, void * secondin, int * len, MPI_Datatype * dptr
) {
  ...
  if (first[1] == -1) {
    ...
    /* do nothing */
    // this comparison MUST be separated from the above
    // otherwise (nonnegative, -1) makes us keep -1
  } else {
    Monomial t(n);
    Monomial u(n);
    for (unsigned i = 0; i < n; ++i) {
      t.set_exponent(i, first[i+3]);
      u.set_exponent(i, second[i+3]);
    }
    keep_second = t > u;
  }
  if (not keep_second) {
...
list<Constant_Polynomial *> buchberger_explorer(
  ...
  const Monomial_Ordering * mord = (*(F.begin()))->monomial_ordering();
  // set up MPI_Datatype
  ...
  // set up MPI_Op
  MPI_Op comparison_op;
  #if (WHICH_COMPARISON == HILBERT_COMPARISON)
    MPI_Op_create(hilbert_chooser, false, &comparison_op);
  #else
    MPI_Op_create(sugar_chooser, false, &comparison_op);
  #endif
  ...
  Critical_Pair_Communication * Pnew;
  ...
  while (min_todo != 0) {
    ...
    uint64_t my_sugar = (R.size() == 0) ? 0 : R.front()->sugar();
    uint64_t curr_sugar;
    MPI_Reduce(&my_sugar, &curr_sugar, 1, MPI_UINT64_T, MPI_MAX, 0, MPI_COMM_WORLD);
    if (comm_id == 0) {
      if (P.size() > 1) 
      ...
      if (curr_sugar == 0 and !P.empty()) {
        curr_sugar = P.front()->sugar();
        for (list<Critical_Pair_XPlor *>::iterator pi = P.begin(); pi != P.end(); ++pi)
          if ((*pi)->sugar() < curr_sugar)
            curr_sugar = (*pi)->sugar();
      }
      cout << "current sugar: " << curr_sugar << endl;
      unsigned num_of_curr_sugar = 0;
      for (list<Critical_Pair_XPlor *>::iterator pi = P.begin();
           pi != P.end();
           ++pi)
      {
        if ((*pi)->sugar() == curr_sugar) ++num_of_curr_sugar;
      }
      number_in_package = 0;
      if (num_of_curr_sugar == 0)
        for (unsigned i = 1; i < comm_size; ++i)
          MPI_Send(&number_in_package, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
      else {
        unsigned distrib_size = num_of_curr_sugar / comm_size + 1;
        Pnew = new Critical_Pair_Communication[distrib_size];
        list<Critical_Pair_XPlor *> Q;
        for (unsigned i = 1; i < comm_size; ++i) {
          number_in_package = 0;
          while (!P.empty() and number_in_package < distrib_size)
          {
            Critical_Pair_XPlor * p = P.front();
            P.pop_front();
            if (p->sugar() != curr_sugar)
              Q.push_back(p);
            else {
              Pnew[number_in_package].first = p->first_index();
              Pnew[number_in_package].second = p->second_index();
              Pnew[number_in_package].sugar = p->sugar();
              ++number_in_package;
              p->set_processor(i);
              Pass.push_back(p);
            }
          ...
          if (number_in_package > 0)
            MPI_Send(Pnew, number_in_package, pair_type, i, 0, MPI_COMM_WORLD);
        }
        number_in_package = 0;
        while (!P.empty() and number_in_package < distrib_size)
        {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    if (comm_id != 0) {
      MPI_Recv(&number_in_package, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      if (number_in_package > 0) {
        Pnew = new Critical_Pair_Communication[number_in_package];
        MPI_Recv(Pnew, number_in_package, pair_type, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
    }
    ...
    for (Critical_Pair_XPlor * p : R) {
      ...
      //double start_time = MPI_Wtime();
      //if (!s->is_zero() and find_reducer(s, G))
      //  reduce_over_basis<vector<Abstract_Polynomial *>>(&s, G, comm_id);
      Abstract_Polynomial * g;
      while (!s->is_zero() and ((g = find_reducer(s, G)) != nullptr)) {
        //cout << comm_id << " reducing s-poly " << *s << endl;
        top_reduce(s, g);
        //cout << comm_id << " reduced to " << *s << endl;
      }
      //r_bcast_time += MPI_Wtime() - start_time;
      p->set_spoly(s);
    }
    ...
    Monomial * wt = nullptr;
      ...
      int64_t wd = -1;
    #endif
    // loop through critical pairs to find optimal pair
    ...
    for (
      ...
      if (s->is_zero()) {
        ...
        #else
          if (wt == nullptr) {
            wt = &(s->leading_monomial()); Rbest = Ri;
            wd = (static_cast<Poly_Sugar_Data *>(s->strategy()))->poly_sugar();
          } else {
            if (*wt > s->leading_monomial()) {
              wt = &(s->leading_monomial()); Rbest = Ri;
              wd = (static_cast<Poly_Sugar_Data *>(s->strategy()))->poly_sugar();
            }
          }
    ...
    /*for (int i = 0; i < comm_size; ++i) {
      if (comm_id == i)
        cout << comm_id << " sending " << wd << endl;
      MPI_Barrier(MPI_COMM_WORLD);
    }*/
    //MPI_Barrier(MPI_COMM_WORLD);
    //double start_time = MPI_Wtime();
    int winners_id = -1;
    // we use -1 if there is no useful data; i.e., all R's polys -> 0
    #if (WHICH_COMPARISON == HILBERT_COMPARISON)
      ...
      if (recv_hp_deg != -1) {
        ...
        MPI_Allreduce(
            send_data, recv_data, xfer_size, MPI_INT64_T, comparison_op,
            MPI_COMM_WORLD
        );
    ...
    #else
      int64_t * send_data = new int64_t[n + 3];
      int64_t * recv_data = new int64_t[n + 3];
      send_data[0] = comm_id; send_data[1] = wd; send_data[2] = n;
      if (wd >= 0) {
        for (unsigned i = 0; i < n; ++i)
          send_data[i + 3] = wt->degree(i);
      }
      MPI_Allreduce(
          send_data, recv_data, n + 3, MPI_INT64_T, comparison_op,
          MPI_COMM_WORLD
      );
      winners_id = (recv_data[1] == -1) ? -1 : recv_data[0];
      delete [] send_data; delete [] recv_data;
      //cout << "winner of comparison: " << winners_id << endl;
    #endif
    ...
    if (winners_id >= 0) {
      ...
      if (comm_id == winners_id) {
        ...
        cout << "selected " << (*Rbest)->first_index()
             ...
             << ',' << sd->poly_sugar() << " from " << winners_id << endl;
